...
public class QuickSort {
       ...
       int pivot_index;
        if (lastIndex-startIndex>1) {
            if (orderBy.equals("compareX")) {
                pivot_index = partitionX(arr, startIndex, lastIndex);
                sort(arr, startIndex, pivot_index - 1, "compareX");
                sort(arr, pivot_index , lastIndex, "compareX");
            }
            else if (orderBy.equals("compareY")) {
                pivot_index = partitionY(arr, startIndex, lastIndex);
                sort(arr, startIndex, pivot_index - 1, "compareY");
                sort(arr, pivot_index , lastIndex, "compareY");
            }
        }
    }
    ...
    private Point2D.Double getMedianX(Point2D.Double[] arr, int left, int right) {
        ...
        // Sort arr[left], arr[right], arr[center]
